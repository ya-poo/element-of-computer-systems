...
class BitTest : FunSpec({
    ...
    test("mux関数") {
        mux(Bit.LOW, Bit.LOW, Bit.LOW) shouldBe Bit.LOW
        mux(Bit.LOW, Bit.HIGH, Bit.LOW) shouldBe Bit.LOW
        mux(Bit.HIGH, Bit.LOW, Bit.LOW) shouldBe Bit.HIGH
        mux(Bit.HIGH, Bit.HIGH, Bit.LOW) shouldBe Bit.HIGH
        mux(Bit.LOW, Bit.LOW, Bit.HIGH) shouldBe Bit.LOW
        mux(Bit.LOW, Bit.HIGH, Bit.HIGH) shouldBe Bit.HIGH
        mux(Bit.HIGH, Bit.LOW, Bit.HIGH) shouldBe Bit.LOW
        mux(Bit.LOW, Bit.HIGH, Bit.HIGH) shouldBe Bit.HIGH
    }

    test("dMux関数") {
        val result1 = dMux(Bit.HIGH, Bit.LOW)
        result1.first shouldBe Bit.HIGH
        result1.second shouldBe Bit.LOW

        val result2 = dMux(Bit.HIGH, Bit.HIGH)
        result2.first shouldBe Bit.LOW
        result2.second shouldBe Bit.HIGH

        val result3 = dMux(Bit.LOW, Bit.LOW)
        result3.first shouldBe Bit.LOW
        result3.second shouldBe Bit.LOW

        val result4 = dMux(Bit.LOW, Bit.HIGH)
